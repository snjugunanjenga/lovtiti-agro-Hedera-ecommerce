...
export interface PaymentRequest {
  ...
  signer?: ethers.Signer; // Preferred for MetaMask/injected wallet
  hederaAccountId?: string; // Optional Hedera account ID
  userId?: string;
}
...
export const processAgroContractPayment = async (request: PaymentRequest): Promise<PaymentResponse> => {
  try {
    ...
    if (!request.productId || !request.walletAddress || !request.userId) {
      return {
        success: false,
        error: 'Missing required fields for agro contract payment: productId, walletAddress, userId',
        status: 'failed'
      };
    }

    // Require either signer or privateKey
    if (!request.signer && !request.privateKey) {
      return {
        success: false,
        error: 'Either signer (preferred) or privateKey must be provided for agro contract payment',
        status: 'failed'
      };
    }
    ...
    const response = await fetch('/api/agro/purchase/buy', {
      ...
      body: JSON.stringify({
        ...
        signer: request.signer, // Send signer for client-side signing
        privateKey: request.privateKey, // Legacy support
