...
export interface WalletConnection {
  ...
  isConnected: boolean;
  privateKey?: string;     // Optional, only for testing/server operations
...
export class WalletManager {
  ...
  async connectMetaMask(): Promise<WalletConnectionResult> {
    try {
      ...
      const wallet: WalletConnection = {
        ...
        signer,
        isConnected: true,
        hederaAccountId: null // Will be set by the hook later
      };
  ...
  async connectWithPrivateKey(privateKey: string, network: 'mainnet' | 'testnet' | 'local' = 'testnet'): Promise<WalletConnectionResult> {
    try {
      ...
      const walletConnection: WalletConnection = {
        ...
        signer: wallet,
        isConnected: true,
        hederaAccountId: null // Will be set by hook later
      };
   ...
   */
  async createFarmerAccount(): Promise<{ success: boolean; error?: string; transactionHash?: string }> {
    if (!this.currentWallet?.signer || !this.currentWallet?.address) {
      return {
        success: false,
        error: 'No wallet connected or signer not available'
      };
    }

    try {
      const result = await this.contractService.createFarmer(this.currentWallet.signer);

      if (result.success) {
        // Log the farmer creation
        console.log(`Farmer created: ${this.currentWallet.address}`, {
...
export const createFarmerAccount = () => walletManager.createFarmerAccount();
