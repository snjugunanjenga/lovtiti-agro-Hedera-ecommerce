...
export default function Navbar() {
  ...
  const fetchFarmerStatus = useCallback(async (targetAddress: string) => {
    ...
    const contract = new ethers.Contract(CONTRACT_ADDRESS, AGRO_CONTRACT_ABI, provider);
    ...
    try {
      ...
      if (farmerStatus.exists) {
        toast({
          ...
          description: `Wallet ${shortAddress(currentWallet.address)} is synced with the contract.`,
        });
        return;
      }
  ...
  const handleCreateProduct = useCallback(async () => {
    ...
    try {
      ...
      const productData = result.data as { productId?: bigint | null } | undefined;
      const productId = productData?.productId;

      toast({
        title: 'Sample product created',
        description: `Wallet ${shortAddress(currentWallet.address)} added product${productId != null ? ` #${productId.toString()}` : ''} at ${samplePrice} HBAR for ${sampleAmount} units.`,
      });
    } catch (error) {
      toast({
        variant: 'destructive',
    ...
    try {
      ...
      if (!products.length) {
        toast({
          ...
          description: `Wallet ${shortAddress(currentWallet.address)} has no products yet.`,
        });
        return;
      }
    ...
    try {
      ...
      if (!info || !info.exists) {
        toast({
          ...
          description: `No farmer record for wallet ${shortAddress(currentWallet.address)}.`,
        });
        return;
      }
      ...
      toast({
        ...
        description: `Balance ${balance} HBAR • ${info.products.length} product${info.products.length === 1 ? '' : 's'}.`,
      });
    } catch (error) {
      toast({
        variant: 'destructive',
                ...
                {isFarmerRole && (
                  <>
                    ...
                    >
                      ...
                      <span>{isRegisteringFarmer ? 'Syncing...' : 'Sync Farmer'}</span>
                    </Button>
                    <Button
                      variant="outline"
                      size="sm"
                    ...
                    >
                      ...
                      <span>{isViewingProducts ? 'Loading...' : 'View Products'}</span>
                    </Button>
                    <Button
                      variant="outline"
                      size="sm"
                    ...
                    >
                      ...
                      <span>{isFetchingFarmerInfo ? 'Checking...' : 'whoFarmer'}</span>
                    </Button>
                  </>
                )}
                <Link href="/settings">
                ...
                <SignedIn>
                  ...
                  {isFarmerRole && (
                    <>
                      ...
                      >
                        {isRegisteringFarmer ? 'Syncing Farmer...' : 'Sync Farmer on Blockchain'}
                      </Button>
                      <Button
                        variant="outline"
                        onClick={handleCreateProduct}
