...
export default function CreateMyAccountPage() {
  ...
  const createAccount = async () => {
    ...
    try {
      ...
      if (data.success && formData.role === 'FARMER') {
        try {
          // If user is a farmer, try to register on blockchain
          const blockchainResponse = await fetch('/api/agro/farmer/create', {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json',
            },
          ...
          if (!blockchainData.success) {
            // If blockchain registration fails, delete the user from database
            await fetch(`/api/auth/delete-user/${data.data.id}`, {
              method: 'DELETE'
            });
            
            setResult({
              success: false,
              message: 'Failed to create farmer account on blockchain',
