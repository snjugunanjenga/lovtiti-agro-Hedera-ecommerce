...
export default function FarmerOnboarding() {
	...
	const handleSubmit = async (e: React.FormEvent) => {
		...
		try {
			...
			// Step 2: Create farmer account on contract if not already a farmer
			if (!isFarmer) {
				console.log('Creating farmer account on contract...');
				const farmerResult = await createFarmerAccount("dummy-id"); // Real userId will come from Clerk
				if (!farmerResult.success) {
					throw new Error(farmerResult.error || 'Failed to create farmer account on contract');
				}
				console.log('Farmer account created on contract successfully!');
				setStatus('Farmer account created on contract successfully!');
			}

			// Step 3: Submit KYC data
			const parsed = kycSchema.safeParse({ ...formData, type: "FARMER" as const });
			if (!parsed.success) {
			...
			const response = await fetch('/api/kyc/submit', {
				...
				body: JSON.stringify({
					...parsed.data,
					walletAddress: wallet?.address // Include connected wallet address
				}),
			});
			...
			console.log('KYC submitted successfully:', kycResult);
							...
							{currentStep === 3 && (
								<div className="space-y-6">
									<div className="bg-green-50 p-4 rounded-lg">
										<h3 className="font-semibold text-green-800 mb-2">Why Connect Your Wallet?</h3>
										<ul className="text-sm text-green-700 space-y-1">
											<li>• Secure blockchain-based transactions</li>
											<li>• Required for farmer registration</li>
											<li>• Manage your products and earnings</li>
